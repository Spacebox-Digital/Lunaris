...
export function Collection({
    ...
    const promoTiles = useMemo(() => {
        ...
        const campaign = promotion.campaigns.find(({collections, enabled}) => {
            ...
            return collections?.some(
                (colHandle) => colHandle.trim() === handle,
            );
        });
    ...
    return (
        ...
        >
            <div
                className={` mx-auto grid w-full max-w-[var(--content-max-width)] !pt-0`}
            >
                {showHeading && (
                    <h1
                        className={`text-h2  mb-4 !pb-0 text-center max-md:px-4 md:mb-2`}
                    >
                        {title}
                    </h1>
                )}
